...
NCIS <- function(chip.data, input.data,
  ...
  shift.size <- round(frag.len/2)     # this is not the binding site length!
  ## only used when binning.

  ncis <- NCIS.internal(chip, input, shift.size=shift.size, min.binsize=min.binsize,
                        max.binsize=max.binsize, binsize.shift=binsize.shift,
                        min.stop.binsize=min.stop.binsize, chr.vec=chr.vec,
                        chr.len.vec=chr.len.vec, quant=quant, keep.bindata=keep.bindata)
  ncis$chip <- chip.data
  ncis$input <- input.data
  class(ncis) <- "NCIS"
  ncis
}                                       #NCIS

plot.bincounts <- function(ncis, main="bin counts",
                           sub=sprintf("normalization factor: %.3f", ncis$est),
                           pch=19, cex=0.8, xlab="input", ylab="chip",
                           est.line.func=function()abline(a=0, b=ncis$est, col="black"),
                           seq.depth.ratio.line.func=function()abline(a=0, b=ncis$r.seq.depth, col="blue"),
                           legend.func=function()legend(x="topleft", legend=c("NCIS estimate","seq.depth estimate"), lty=1, col=c("black", "blue")),
                           ...) {
  ## bit like Liang & Keles Fig4
  if(!is(ncis,"NCIS"))
    warning("not an NCIS object (maybe old version?)")
  if(is.null(ncis$bindata))
    stop("No bindata present in NCIS object (should have called NCIS with keep.bindata=TRUE)")

  with(ncis$bindata, 
       heatscatter(x=input,y=chip, pch=pch,cex=cex,
                   main=main, sub=sub, xlab=xlab, ylab=ylab, ...))

  if(is.function(seq.depth.ratio.line.func))
    seq.depth.ratio.line.func()

  if(is.function(est.line.func))
    est.line.func()

  if(is.function(legend.func))
    legend.func()
}                                       #plot.bincounts

#NCIS takes chip.pos and input.pos
NCIS.internal <- function(chip.pos, input.pos,
                          shift.size=100,
                          min.binsize=100, max.binsize=20000, 
                          binsize.shift=100, min.stop.binsize=100,
                          chr.vec=NULL, chr.len.vec=NULL, quant=0.75,
                          keep.bindata=FALSE) {
    ...
    norm.est <- rep(1000, length(sizevec)) #initial estimate
    #names(norm.est) <- sizevec

    ## get ends of chromosomes. If not given, take the largest coordinate found:
...
read.BED <- function(bed.file){
    ...
    tt <- temp[index[1],]
    if(tt$pos < tt$posEnd)              # make sure that pos is always that of read-end
      temp$pos[index] <- temp$posEnd[index]

    ## now make list of just the end-points, nested by chromo, then by strand
